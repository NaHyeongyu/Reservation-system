...
add column if not exists applicant_instagram_id text;
...
begin
  if not exists (
    select 1
    from pg_constraint
    where conname = 'reservations_applicant_instagram_id_check'
  ) then
    alter table public.reservations
    add constraint reservations_applicant_instagram_id_check
    check (
      applicant_instagram_id is null
      or applicant_instagram_id ~ '^[A-Za-z0-9._]{1,30}$'
    );
  end if;
end
$$;
