...
import { createBranchAdminAction } from "../_actions/create-branch-admin";

const initialState = {
  errorMessage: null,
};

export function BranchAdminCreateForm({ branchId }: { branchId: string }) {
  const [state, formAction] = useActionState(createBranchAdminAction, initialState);
  ...
  return (
    <form action={formAction} className="grid gap-5 rounded-[30px] border border-[#1c2733] bg-[#0b141d] p-6 shadow-[0_24px_80px_rgba(0,0,0,0.22)]">
      ...
      <Field label="아이디" htmlFor="loginId">
        <input id="loginId" name="loginId" type="text" className="w-full rounded-[18px] border border-[#223140] bg-[#0f1822] px-4 py-3.5 text-base text-white outline-none transition focus:border-[#7ad0ff]" required />
      </Field>
      <Field label="비밀번호" htmlFor="password">
        <input id="password" name="password" type="password" className="w-full rounded-[18px] border border-[#223140] bg-[#0f1822] px-4 py-3.5 text-base text-white outline-none transition focus:border-[#7ad0ff]" required />
      </Field>
